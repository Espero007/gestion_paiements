...
$nom = htmlspecialchars($banques[0]['nom']);
$prenoms = htmlspecialchars($banques[0]['prenoms']);
$acteur = '<a href="gerer_participant.php?id=' . chiffrer($id_participant) . '">' . $nom . ' ' . $prenoms . '</a>'; // pour afficher le nom plus facilement
        ...
        if (isset($_POST['banque'])) {
            ...
            foreach ($comptes_choisis as $compte) {
                if (in_array($compte, $numeros_comptes)) {
                    ...
                    if ($stmt->rowCount() != 0) {
                        ...
                        $stmt->closeCursor();
                        $erreurs['compte_deja_utilise'] = 'Vous ne pouvez pas supprimer le compte (<strong>' . strtoupper($compte) . '</strong>) car ' . $acteur . ' a été associé à l\'activité "<strong>' . htmlspecialchars($resultat['nom']) . '</strong>" avec ce dernier. <a href="/gestion_participants/liaison.php?modifier=' . chiffrer($resultat['id']) . '&s=0">Cliquez ici</a> pour modifier les informations de cette liaison puis revenez supprimer le compte.';
                    }
                } else {
                    redirigerVersPageErreur(404, $_SESSION['previous_url']);
                }
            ...
            if (!isset($erreurs)) {
                foreach ($comptes_choisis as $compte) {
                    ...
                    // On supprime enfin la ligne dans la table informa-tions_bancaires
                    $stmtDeleteCompte = $bdd->prepare("DELETE FROM informations_bancaires WHERE id = ?");
                    $stmtDeleteCompte->execute([$id]);
                }

                arrangerRibs($id_participant); // Pour renommer adéquatement les pdfs_ribs, s'il en reste bien-sûr

                // On redirige vers la page précédente avec un message de succès
                $participant = $nom . ' ' . $prenoms;
