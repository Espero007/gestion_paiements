...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($data['taux_taches'])) $errors['taux_taches'] = "Le taux par tâche est requis.";
    if (empty($data['frais_de_deplacement'])) $errors['frais_de_deplacement'] = "Les frais de  déplacement journalier sont requis.";
    ...
    if ($data['taux_taches'] !== '') {
        if (filter_var($data['taux_taches'], FILTER_VALIDATE_FLOAT) === false) {
            $errors['taux_taches'] = "Le taux des tâches doit être un nombre décimal valide (ex. : 123.45).";
        }
    }

    if ($data['frais_de_deplacement'] !== '') {
        if (filter_var($data['frais_de_deplacement'], FILTER_VALIDATE_FLOAT) === false) {
            $errors['frais_de_deplacement'] = "Les frais de déplacement doivent être un nombre décimal valide (ex. : 123.45).";
        }
    }
    ...
    $numerical_fields = ['taux_taches', 'frais_de_deplacement'];
    ...
    if (empty($errors)) {
        ...
        $stmt->execute([
            ...
            'taux_taches' => $data['taux_taches'] ,
            'frais_deplacement' => $data['frais_de_deplacement']
...
<?php  //echo "<h2> Bienvenue sur notre site <i style = 'color:blue'>". $loggedUser["Nom"] . " ". $loggedUser["Prenom"] ."</i> </h2>"; ?>
    ...
    <h2>Création d'une Activité de type 3</h2>
    <?php if ($success): ?>
